...
from jobflow import Flow, run_locally, job, Response
...
def parallel_eos_calculation(
    ...
    flow = Flow(eos_jobs)
    return Response(output=eos_outputs, addition=flow)
...
def eos_workflow(element, configuration, ecut, pseudo_names, volume_scaling_list=None, precision='standard'):
    """
    :param element: str, the name of element
    :param configuration: str, it should be one of the config in
    ["XO", "XO2", "XO3", "X2O", "X2O3", "X2O5", "BCC", "FCC", "SC", "DC"]
    :param ecut: float, Ecut (unit: Ha)
    :param pseudo_names: str| [str], name of pseudos file, such like H.psp8, Li.upf,
    location of pseudos should be specified at env *EOS_PSEUDO_PATH*
    ...
    if volume_scaling_list is None:
        volume_scaling_list = [0.94, 0.96, 0.98, 1.00, 1.02, 1.04, 1.06]
    if isinstance(pseudo_names, str):
        pseudo_names = [pseudo_names]
    pps_job = get_remote_pseudo_path(pseudo_names)
    eos_jobflow.append(pps_job)

    structure = get_standard_structure(element, configuration)
    inputs_job = generate_abinit_inputs(element, configuration, ecut, pps_job.output, structure, precision=precision)
    eos_jobflow.append(inputs_job)

    eos_jobs = parallel_eos_calculation(
        structure, pps_job.output,
        abinit_settings=inputs_job.output["user_abinit_settings"],
        kpoints_settings=inputs_job.output["user_kpoints_settings"],
